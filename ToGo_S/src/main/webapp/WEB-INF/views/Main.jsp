...
<body>
	...
	<div id="wrapper">
		...
		<%if (loginMember == null) {%>
		 ...
		 <% } else { %>
		  ...
		  <% if (loginMember.getEmail().equals("admin")) { %>
		 	...
		 	<% } %>
		 <% } %>
		 <!-- Q7. 개인정보수정 기능 만들기 --> <!-- Q8. 로그아웃 기능 만들기 -->
		<!-- Q9. 관리자 계정(admin)일 때는 회원정보관리 탭 만들기 --> </nav> </header>
		...
		<ul class="actions vertical">
			...
			<form action="memberInsert" method="post">
				...
				<li><input type="button" value="Email 중복체크" onclick="checkE()"></li>
		...
		<div class="inner">
			...
			<div class="content">
				...
				<ul class="actions">
					<li><a href="goBoardMain" class="button next scrolly">게시판
							바로가기</a></li>
				</ul>
			</div>
		</div>
		...
		<div id="main">
			...
			<section id="one" class="tiles"> <article> <span
				class="image"> <img src="resources/images/pic01.jpg" alt="" />
			...
			<div class="inner">
				...
				<ul class="actions">
					...
					<li>로그인을 하세요.</li>
					<!-- Q14. 메시지 전체 삭제 기능 -->
					<li><a href="deleteAllMessage?email=${loginMember.email }" class="button next scrolly">전체삭제하기</a></li>
				</ul>
				...
				%>
				<table>
					<%for(Message m:m_list){ index++; %>
					<tr>
						<td><%=index %></td>
						<td><%=m.getSendE() %></td>
						<td><%=m.getMessage() %></td>
						<td><%=m.getM_date() %></td>
						<td><a href = "deleteMessage?num=<%=m.getNum() %>">삭제</a></td>
					</tr>
					<%} %>
				</table>
				<%} %>
		...
		<div class="inner">
			...
			<form action="messageSend">
				<div class="field half first">
					...
					<%if(loginMember == null){ %>
						<input type="text" name="sendE" id="name" placeholder="보내는 사람 이름" />
					<%}else{ %>
						<input type="text" name="sendE"  value="${loginMember.getEmail() }" id="name" placeholder="보내는 사람 이름" />
					<%} %>
					 
				</div>
				<div class="field half">
					<label for="email">Email</label> 
					<input type="text" name="receiveE" id="email" placeholder="받는 사람 이메일" onclick="getMemberEmail()"/>
				</div>
	...
	<script type="text/javascript">
		function checkE() {
			...
			$.ajax({/* JSON(JavaScript Object Notaion): 자바객체 표현 방식*/
				...
				success : function(data) {
					if (data == 1) {
						$('#resultCheck').text('사용가능한 email')
					} else {
						$('#resultCheck').text('사용 불가능한 email')
					}
				},
				error : function() {
					alert("통신실패!")
